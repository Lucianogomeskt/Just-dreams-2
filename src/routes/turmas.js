...
function gerarCodigoTurma(nome) {
  // Pegar as primeiras letras de cada palavra do nome
  const palavras = nome.trim().split(/\s+/);
  let codigo = palavras.map(palavra => palavra.charAt(0).toUpperCase()).join('');
...
router.post('/', requireProfessor, async (req, res) => {
  try {
    ...
    if (nome.length < 2 || nome.length > 100) {
      return res.status(400).json({
        success: false,
        message: 'Nome da turma deve ter entre 2 e 100 caracteres'
      });
    }
  ...
  try {
    ...
    res.status(200).json({
      ...
      data: {
        ...
        total: professor.turmas ? professor.turmas.length : 0
      }
    });
