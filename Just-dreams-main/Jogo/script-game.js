document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!token || user.tipo !== 'aluno') {
        ...
        window.location.href = '../JustDreams/aluno_login.html';
        return;
    }

    // Carregar informações do personagem e profissão escolhidos
    const personagemEscolhido = localStorage.getItem('personagemEscolhido');
    const profissaoEscolhida = localStorage.getItem('profissaoEscolhida');
    
    if (!personagemEscolhido || !profissaoEscolhida) {
        alert('Por favor, escolha seu personagem e profissão primeiro!');
        window.location.href = '../JustDreams/escolha_profissao.html';
        return;
    }
    ...
    // Inicializar fase atual se não existir
    ...
    async function endGame() {
        ...
        // Marcar que o jogo foi completado
        localStorage.setItem('jogoCompletado', 'true');
        
        // Avançar para próxima fase
        const faseAtual = parseInt(localStorage.getItem('faseAtual') || '1');
        localStorage.setItem('faseAtual', (faseAtual + 1).toString());
    ...
    document.getElementById('advance-button').addEventListener('click', () => {
        // Redirecionar para a jornada do personagem
        window.location.href = '../JustDreams/jornada_personagem.html';
    });
